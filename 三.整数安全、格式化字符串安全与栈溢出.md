...
>
> x86和ARM中存在一些指令序列，能够代替ret的工作，它们首先更新全局状态（如栈指针），然后根据更新后的状态加载下一条指令地址，并跳转执行。这样的指令序列被称为update-load-branch，使用它们来代替ret指令。但update-load-branch一般比ret指令稀少，通常作为跳板（trampoline）使用，当一个gadget执行结束后，跳转到trampoline，trampoline更新程序状态后把控制权交给下一个gadget，由此行程ROP链。
