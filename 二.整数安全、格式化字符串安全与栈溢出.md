...
#### 示例6-只能打一次的格式化字符串 无PIE .init_array可写 有system（**Partial RELRO**）
...
* 可以打的方式有很多，由于格式化字符串在栈上，即使只能打一次，只要足够长，可以往任意地址读写、
...
#### 示例7-只能打一次的格式化字符串 无PIE 无system .init_array可写（**Partial RELRO**）
...
#### 示例8-只能打一次的格式化字符串 有PIE 无system .init_array可写（**Partial RELRO**）
